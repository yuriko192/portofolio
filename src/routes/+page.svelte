...
<style lang="scss">
  ...
  #blob {
    ...
    translate: -50% -50%;
    border-radius: 50%;
    background: Gainsboro linear-gradient(to right, SeaGreen, DarkOrchid);
...
<div class="fixed dark:bg-zinc-800 bg-zinc-50 w-screen h-screen z-[-99]">
	<!--    MOVING RAINBOW BLOB-->
	{#if isDarkMode}
...
<div class="dark:text-slate-100 px-8 py-2 min-h-screen overflow-hidden" onpointermove={handleBlobMove}>
	...
	<div class="flex mb-4 mt-4 gap-4s">
		...
		<div class="flex justify-items-center items-center mr-4">
			<a href="https://github.com/Ivan-Wij"
               ...
               rounded-lg py-1 px-3 hover:shadow
               bg-transparent
               dark:hover:text-zinc-700 dark:hover:bg-zinc-300 dark:text-white dark:border-white
               hover:bg-zinc-800 hover:text-zinc-300 text-black border border-black
               transition">
				<Github />
				Github
			</a>
		...
		<div class="flex justify-items-center items-center mr-4">
			<a href="https://drive.google.com/file/d/1uVZdRZIvT5WF19So0d63yTEizPci3YvQ/view?usp=sharing"
               ...
               dark:hover:bg-zinc-400 dark:bg-zinc-200 dark:text-black dark:font-semibold dark:shadow-zinc-500
               hover:bg-zinc-600 bg-zinc-800 text-white
               transition">
				Get CV
			</a>
		</div>
	...
	<div class="flex py-5 align-middle flex-col sm:flex-row">
		<div class="shrink-0 m-auto mb-8 px-8">
			<img class="shadow-md dark:shadow-zinc-600 rounded-full w-full max-w-[32rem] aspect-square" src={selfImg}
					 alt="ivan wijaya">
		</div>
	...
	<div class="my-4 grid gap-4 grid-flow-row-dense sm:grid-flow-col-dense grid-cols-4 grid-rows-2 lg:grid-rows-1">
		{#each subJobs as subJob, idx}
			...
			<div class={`rounded shadow-md py-2 px-4 bg-transparent border-solid border
						bg-zinc-50 dark:bg-zinc-800 dark:bg-opacity-40
            dark:border-gray-200  border-slate-700
            dark:hover:border-gray-400 hover:border-slate-400 hover:shadow-none
             	text-xl font-bold ${subJob.spans}`}
